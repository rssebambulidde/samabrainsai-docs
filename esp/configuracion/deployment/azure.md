---
description: Learn how to deploy SamaFlow on Azure
---
...
This setup makes deploy SamaFlow on Azure App Service with a PostgreSQL Flexible Server. It's based on the best practices of Azure for web application deployments.
...
1. Clone the SamaFlow repository:
...
// File share
resource "azurerm_storage_share" "samaflow-share" {
  name                 = "samaflow"
...
variable "flowise_secretkey_overwrite" {
  type        = string
  sensitive   = true
  description = "SamaFlow secret key"
}
...
variable "flowise_image" {
  type        = string
  description = "SamaFlow image from Docker Hub"
}

variable "tagged_image" {
  type        = string
  description = "Tag for samaflow image version"
}
```
...
resource "azurerm_linux_web_app" "webapp" {
  ...
  storage_account {
    ...
    share_name   = azurerm_storage_share.samaflow-share.name
    type         = "AzureFiles"
    mount_path   = "/root"
  }
...
2. Select or create a Resource group, Container name, Region, Image source `Other registry`, Image type, Image `flowiseai/samaflow`, OS type and Size. Then click "Next: Networking" to configure SamaFlow ports:
...
4. Set Restart policy to `On failure`. Add Command override `["/bin/sh", "-c", "samaflow start"]`. Finally click "Review + create":
...
7. Visit your SamaFlow instance by copying IP address and adding :3000 as a port:
...
<figure><img src="../../.gitbook/assets/azure/deployment/8.png" alt=""><figcaption><p>SamaFlow application deployed as Container Instance</p></figcaption></figure>
