...
* Using [Human Input](../using-samaflow/agentflowv2.md#id-11.-human-input-node) node to halt the execution
...
2.  Add system message, for example:
    ...
    ```
    You are a customer support agent working in SamaFlow Inc. Write a professional email reply to user's query. Use the web search tools to get more details about the prospect.

    Always reply as Samantha, Customer Support Representative in SamaFlow. Don't use placeholders.
    ```
...
2. If tool calls are identified from LLM output response, SamaFlow locates the matching tools and executes the corresponding functions.
...
2.  Add a system message to Agent, for example:
    ...
    ```
    You are a customer support agent working in SamaFlow Inc. Create a draft professional email reply to user's query. Use the web search tools to get more details about the prospect.
...
4. Users outside of SamaFlow can reject or approve:
