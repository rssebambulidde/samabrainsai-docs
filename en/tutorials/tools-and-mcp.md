...
In the previous [**Interacting with API**](interacting-with-api.md) tutorial, we explored how to enable LLMs to call external APIs. To enhance the user experience, SamaFlow provides a list of prebuilt tools. Refer to the [**Tools**](../integrations/langchain/tools/) section for the full list of available integrations.
...
* You can use any libraries imported in SamaFlow.
...
* **Hosts** are LLM applications (like SamaFlow) that initiate connections
...
Only use this when using SamaFlow locally, not when deployed to cloud services. This is because running command like `npx` will install the MCP server package (ex: `@modelcontextprotocol/server-sequential-thinking`)  locally, and it often takes long time for that.&#x20;
...
The Docker command is suitable when the machine running SamaFlow also has access to Docker. However, it is not suitable for deployments on cloud services where Docker access is restricted or unavailable.
...
Create an Agent node, and add a new Custom MCP tool. For streamable HTTP, we just need to put in the URL and other necessary headers. You can use [variables](../using-samaflow/variables.md) in the MCP Server Config with double curly braces `{{ }}` and prefix `$vars.<variableName>`.
...
If the MCP server configuration is working correctly, you can refresh the **Available Actions**, and SamaFlow will automatically pull in all available actions from the MCP server.
