...
[Functions](https://docs.openwebui.com/features/plugin/functions/) are like plugins for Open WebUI. We can create a custom [Pipe Function](https://docs.openwebui.com/features/plugin/functions/pipe) that process inputs and generate responses by invoking SamaFlow Prediction API before returning results to the user. Through this, SamaFlow can be used in Open WebUI.
...
```python
"""
title: SamaFlow Integration for OpenWebUI
Requirements:
  - SamaFlow API URL (set via FLOWISE_API_URL)
  - SamaFlow API Key (set via FLOWISE_API_KEY)
"""
...
class Pipe:
    class Valves(BaseModel):
        flowise_url: str = Field(
            ...
            description="SamaFlow URL",
        )
        flowise_api_key: str = Field(
            default=os.getenv("FLOWISE_API_KEY", ""),
            description="SamaFlow API key for authentication",
        )
    ...
    def __init__(self):
        ...
        if not self.valves.flowise_url:
            print(
                "⚠️ Please set your SamaFlow URL using the FLOWISE_API_URL environment variable"
            )
        if not self.valves.flowise_api_key:
            print(
                "⚠️ Please set your SamaFlow API key using the FLOWISE_API_KEY environment variable"
            )
    ...
    def pipe(
        ...
        try:
            ...
            # model can be "samaflow.<id>" or just "<id>"
            ...
            with requests.post(
                ...
                if stream_enabled:
                    ...
                    for raw_line in r.iter_lines(decode_unicode=True):
                        ...
                        # SamaFlow usually sends {"event":"token","data":"..."}
                        try:
            ...
            try:
                ...
                detail = ""
            return f"HTTP error from SamaFlow: {http_err.response.status_code} {detail}"
        except Exception as e:
            return f"Error in SamaFlow pipe: {e}"
```

4. After Function has been saved, enable it, and click the settings button to put in your SamaFlow URL and SamaFlow API Key:
