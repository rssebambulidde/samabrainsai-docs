...
3. Name your client (e.g., `SamaFlow Agent`) and click **Create**.
...
### 2.3 Add Credentials in SamaFlow
...
1. Openan agentflow in the SamaFlow canvas.
2. Add an **Agent** node.
...
| **User ID**            | Text (accepts variables) | Yes      | A unique identifier for your end user. Supports SamaFlow variables `{{$vars.user_email}}` and flow variables `{{$flow.sessionId}}`. See [Section 7](pipedream-mcp-user-guide.md#7-using-variables-for-user-id).                               |
...
| ---------------------- | --------------------------- | --------------------- |
| `{{$vars.user_email}}` | SamaFlow workspace variables | Design-time + Runtime |
| `{{$flow.sessionId}}`  | Flow context (session)      | Runtime only          |

### Workspace Variables

1. In SamaFlow, go to **Variables** from the sidebar.
...
| **Protect your Client Secret**         | Never expose the Client Secret in client-side code or version control. SamaFlow stores it encrypted.                   |
