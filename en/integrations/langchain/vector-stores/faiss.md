...
### Method 2: Docker
If you are running SamaFlow using Docker, Faiss is ready to use.
...
* **Base Path:** The folder path where the index files (`faiss.index` and `docstore.json`) will be saved.
    * If left blank, data is stored in RAM and will be lost when SamaFlow restarts.
    * **Docker Path Example:** `/root/.samaflow/faiss`
