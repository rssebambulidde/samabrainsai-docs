---
description: Learn how to deploy SamaFlow on AWS
---
...
Two options are available to deploy SamaFlow on AWS:
...
It deploys SamaFlow on an ECS cluster exposed through ELB.
...
Feel free to edit this template to adapt things like SamaFlow image version, environment variables etc.

Example of command to deploy SamaFlow using the [AWS CLI](https://aws.amazon.com/fr/cli/):

```bash
aws cloudformation create-stack --stack-name samaflow --template-body file://samaflow-cloudformation.yml --capabilities CAPABILITY_IAM
```

After deployment, the URL of your SamaFlow application is available in the CloudFormation stack outputs.

## Deploy on ECS using Terraform

The Terraform files (`variables.tf`, `main.tf`) are available in this GitHub repository: [terraform-samaflow-setup](https://github.com/huiseo/terraform-samaflow-setup/tree/main).

This setup deploys SamaFlow on an ECS cluster exposed through an Application Load Balancer (ALB). It is based on AWS best practices for ECS deployments.

You can modify the Terraform template to adjust:

* SamaFlow image version
...
4. <pre class="language-bash"><code class="lang-bash"><strong>sudo nano /etc/nginx/conf.d/samaflow.conf
   </strong></code></pre>
...
You have successfully setup SamaFlow apps on EC2 instance with SSL certificate on your domain[🥳](https://emojipedia.org/partying-face/)
