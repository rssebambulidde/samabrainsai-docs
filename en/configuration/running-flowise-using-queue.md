...
* [**RedisEventPublisher**](https://github.com/SamaFlow/SamaFlow/blob/main/packages/server/src/queue/RedisEventPublisher.ts) broadcasts events from worker to Redis
* [**RedisEventSubscriber**](https://github.com/SamaFlow/SamaFlow/blob/main/packages/server/src/queue/RedisEventSubscriber.ts) forwards events from Redis to SSE clients
* [**SSEStreamer**](https://github.com/SamaFlow/SamaFlow/blob/main/packages/server/src/utils/SSEStreamer.ts) delivers events to browser in real-time
...
This is the same as you were to run SamaFlow by default, with the exceptions of configuring the environment variables mentioned above.
...
# View logs
docker compose -f docker-compose-queue-prebuilt.yml logs -f samaflow
docker compose -f docker-compose-queue-prebuilt.yml logs -f samaflow-worker
```

### Method 2: Build from Source

This method builds SamaFlow from source code, useful for development or custom modifications.

**Step 1: Setup Environment**

Create the same `.env` file as in [Method 1](running-samaflow-using-queue.md#method-1-pre-built-images-recommended).
...
* Build the main SamaFlow application from source
...
Set `ENABLE_BULLMQ_DASHBOARD` to true will allow users to view all the jobs, status, result, data by navigating to `<your-samaflow-url.com>/admin/queues`
